...
Проект включает в себя набор связанных между собой образовательных и аналитических баз данных, каждая из которых моделирует бизнес-процессы и операции в различных отраслях.

Каждая база используется для отработки навыков сложных SQL-запросов, анализа данных и построения иерархий с использованием `WITH RECURSIVE`, `GROUP_CONCAT`, `JOIN`, оконных функций и агрегатов.

---

## Цели проекта

- Построение логически связных баз данных.
- Создание запросов для аналитики: рейтинг, загрузка, иерархия, предпочтения.
- Решение задач с реальными условиями (например, поиск лидеров, клиентов, подчинённых).
- Формирование SQL-отчетов по различным ролям: менеджеры, клиенты, участники проектов.
...
### 1. Транспорт и гонки
- `Vehicle`, `Car`, `Motorcycle`, `Bicycle` — модели транспорта.
- `Classes`, `Cars`, `Races`, `Results` — участие в гонках и их результаты.

### 2. Отели и бронирования
- `Hotel`, `Room`, `Customer`, `Booking` — информация о гостиницах, клиентах и бронях.

### 3. Сотрудники и иерархия
- `Employees`, `Departments`, `Roles`, `Projects`, `Tasks` — структура компании, связи, роли и задачи.

---

## Используемые конструкции

- `WITH RECURSIVE` — построение иерархий.
- `GROUP_CONCAT` — объединение данных в один столбец.
- `JOIN`, `LEFT JOIN`, `COUNT`, `SUM`, `AVG` — классическая аналитика.
- `CASE`, `FIELD`, `HAVING`, `UNION` — для условий, ранжирования и объединения данных.
